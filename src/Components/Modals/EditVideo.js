...
import { Modal, Form, Input, InputNumber, Button, DatePicker, Image, Row } from "antd";
...
const EditVideo = ({ onCancel, onOk, initialValues = {}}) => {
  ...
  if (!bgUpdated) {
    setBgUpdated(true);
    setBgUrl(initialValues.background);
  }
  if (!thumbUpdated) {
  ...
  return (
    <Modal title="Edit Video" visible={true} footer={null} onCancel={onCancel} okText="Save">
      <Form {...layout} onFinish={(values) => onOk(values)} validateMessages={validateMessages} initialValues={initialValues} onValuesChange={onValuesChange}>
        <Form.Item
          name={"title"}
          label="Title"
          rules={[
